...
- [Overview](#overview)
- [🚚 The Courier Analogy](#-the-courier-analogy)
...
> The LLM is the courier — it carries your question to an answer.
> The graph + vector store is the road — a broken road means the courier can't deliver.
...
| Question | Vector Search Alone | GraphRAG | 🚚 Courier |
|----------|---------------------|----------|-----------|
| "What is RAG?" | ✅ Finds chunks about RAG | ✅ Finds chunks + expands to "Embeddings", "Retrieval", "Vector DB" | A factoid question — GPS alone gets the right parcels, but the paper map throws in the neighbouring towns of "Embeddings" and "Vector DB" for richer context |
...
| "What did we learn from rag-chatbot?" | ❌ Scattered chunks | ✅ Topic "rag-chatbot" links to all sub-topics | A roll-up question — GPS finds 5 random rag-chatbot chunks; the paper map fans out from the rag-chatbot town to every connected sub-topic, so the courier collects the full set |
...
[4] LLM Completion (Ollama / Bedrock / Azure OpenAI)
     → system prompt: COURIER_SYSTEM_PROMPT (always on)
     → generates answer + 🚚 courier analogy
     ↓
[5] Response
     → answer, courier_analogy, sources, topics, latency_ms
     ↓
[6] Feedback Loop
...
Week N: The road is well-paved. Courier delivers fast and accurately.
```
...
| Component | Local | AWS | Azure | 🚚 Courier |
|-----------|-------|-----|-------|-----------|
| LLM | Ollama (free) | Bedrock Haiku ($0.00025/1K tokens) | GPT-4o-mini ($0.00015/1K tokens) | The writer at the desk — a free local llama vs a per-token Claude Haiku vs a per-token GPT-4o-mini, same shipping manifest format |
| Embeddings | nomic-embed-text (free) | Titan v2 ($0.00002/1K tokens) | text-embedding-3-small ($0.00002/1K tokens) | The GPS-coordinate stamper — turns each parcel into a numeric address; local stamps are free, cloud stamps charge per 1K tokens stamped |
| Vector Store | ChromaDB (free) | DynamoDB PAY_PER_REQUEST | Azure AI Search (free tier) | The GPS warehouse — a local barn shelf, an AWS pay-per-fetch depot, or an Azure free-tier hub holding the same parcels |
...
| **Total/run** | **€0** | **~€0.25-0.50** | **~€0.50-1.00** | The full fuel-and-toll bill for one lab session of ~50 deliveries, all-in across writer, stamper, warehouse, and map |
