...
| Phase / Lab | Knob | Yardstick metrics primarily affected | 🫏 Donkey |
| --- | --- | --- | --- |
| config-tuning L1 | `RAG_CHUNK_SIZE` (200 / 500 / 1000) | retrieval, completeness | How big each backpack pocket is |
...
| config-tuning L3 | `RAG_TOP_K` (1 / 3 / 5 / 10) | retrieval, latency, cost | How many backpacks the donkey carries on each trip |
| config-tuning L4 | `LLM_TEMPERATURE` (0.0 / 0.3 / 0.7) | faithfulness, hallucination_rate | How creative the donkey is allowed to be when copying from the backpacks |
| config-tuning L5 | `SYSTEM_PROMPT` (strict vs lax) | faithfulness, hallucination_rate | The standing order on the delivery note — "ONLY use the cargo" vs "use cargo plus your own judgement" |
...
| config-tuning L7 | reranker on/off | retrieval (precision↑), latency (slight↑) | A second-pass quality check at the warehouse before the donkey leaves |
| config-tuning L8 | hybrid search on/off (vector + BM25) | retrieval on exact-string queries | GPS plus an old-school keyword radio — useful when the delivery note has codes |
| config-tuning L9 | `LLM_MAX_TOKENS` | answer completeness, cost | How much cargo the reply truck can carry back |
| config-tuning L10 | distance metric (cosine / dot / L2) | retrieval | Which ruler measures GPS-closeness |
| config-tuning L11–13 | HNSW M, ef_construction, ef_search | retrieval, latency trade-off | How dense the warehouse's stadium-sign network is, and how many signs the donkey checks per trip |
| config-tuning L14 | query rewriting on/off | retrieval | Whether the dispatcher rewrites vague delivery notes before sending the donkey |
| config-tuning L15 | multi-query (n rephrasings) | retrieval, cost | Asking the question N different ways and merging the results |
| config-tuning L16 | metadata filtering | retrieval precision | Pre-sorting the warehouse aisle before the donkey enters |
...
| Phase 1 L1 | vector search baseline (no graph) | retrieval, latency | Donkey using GPS only, no paper map |
| Phase 1 L2 | graph traversal depth (1 / 2 hops) | graph_completeness, latency | How many neighbouring towns the donkey visits per trip |
| Phase 1 L3 | GraphRAG combined (vector + graph) | retrieval + completeness | GPS plus paper map — the full GraphRAG run |
| Phase 2 L4 | LLM-as-Judge faithfulness | faithfulness measurement quality | A second donkey grading the first donkey's report card |
...
| Phase 2 L6 | completeness scoring | completeness | Did the donkey deliver every item on the order, or just some? |
| Phase 2 L7 | hallucination rate sweep | hallucination_rate | How often the donkey invents packages |
| Phase 2 L8 | cost per query | $/query | Hay-and-toll cost per delivery, normalised per 1k trips |
...
| Phase 3 L11 | provider sweep (local / AWS / Azure) | all metrics × 3 | Same race, three different stables — who wins on which subject |
| Phase 3 L12 | CI/CD eval gate | regression detection | The robot stable hand refusing to ship a slower or less faithful donkey |
...
🫏 **Final donkey wisdom:** every lab is one of these two questions, dressed up differently:

- "I changed knob X. Is the report card better or worse?" (Phase 1 L1–L3, Phase 2 L4–L7, config-tuning L1–L10)
- "I changed knob X. Did the donkey now refuse / crash / answer slower / spend more?" (config-tuning L11–L17, Phase 3 L11–L12)
